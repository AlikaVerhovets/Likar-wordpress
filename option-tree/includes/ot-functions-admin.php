          ...
          array(
            ...
            'pages'               => array(
              array(
                ...
                'button_text'     => apply_filters( 'ot_theme_options_button_text', __( 'Сохранить', 'option-tree' ) ),
  ...
  function ot_register_settings_page() {
    ...
    $ot_register_pages_array =  array(
      ...
      array(
        ...
        'button_text'     => __( 'Сохранить', 'option-tree' ),
        'show_buttons'    => false,
        'sections'        => array(
          array(
            'id'          => 'create_setting',
...
if ( ! function_exists( 'ot_list_item_view' ) ) {
  ...
  function ot_list_item_view( $name, $key, $list_item = array(), $post_id = 0, $get_option = '', $settings = array(), $type = '' ) {
    ...
    $settings = array_merge( $required_setting, $settings );
	  ...
	  //Получаем значение поля "Дата создания"
		if (isset( $list_item['analize_create'] ) ){

		  $dayCount=0;

		  $createDate=$list_item['analize_create'];

			
			
		  $now = new DateTime(); // текущее время на сервере
		  $date = DateTime::createFromFormat("Y-m-d H:i", $createDate); // задаем дату создания анализа
		  $interval = $now->diff($date); // получаем разницу в виде объекта DateInterval
		  $dayCount=($interval->d)+($interval->m*30)+($interval->y*365);
			


				  if ($dayCount>3) {
					  $isEdit=' style="display:none;" ';
					  $newDiv='<div class="z-index"></div>';
				  } else {
					  $isEdit='';
					  $newDiv='';
				  }
    ...
    <div class="option-tree-setting">
      ...
      <div class="button-section">
        ...
        <a href="javascript:void(0);"'.$isEdit.' class="option-tree-setting-remove option-tree-ui-button button button-secondary light right-item" title="' . __( 'Delete', 'option-tree' ) . '">
          <span class="icon ot-icon-trash-o"></span>' . __( 'Delete', 'option-tree' ) . '
        </a>
      </div>
      <div class="option-tree-setting-body">
      ...
      foreach( $settings as $field ) {
		...
		//Получаем значение поля "Дата создания"
		if ($field['id']=='analize_create'){

		  $createDate=$field_value;
